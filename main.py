...
def predict_score(
    test_data_len: int, train_data: list, test_data: list, k: int
) -> float:
...
def test_track(track_file: str) -> str:
    ...
    # for i, folder in enumerate(listdir(directory), 1):
    # 	print(f"Reading directory: {folder}")
    ...
    # 		)  # Read sample_rate and signal data from file
    # 		mfcc_feat = mfcc(
    # 			sig, rate, winlen=0.020, appendEnergy=False
    # 		)  # Generate feature matrix from signal and sample_rate
    # 		covariance = np.cov(np.matrix.transpose(mfcc_feat))
    # 		mean_matrix = mfcc_feat.mean(0)
    # 		feature = (mean_matrix, covariance, i)
...
def main() -> None:
    ...
    # Generate DAT file if it does not exist
    ...
    # training_data, test_data = load_dataset("./feat.dat", 0.66)

    # model_prediction = predict_score(len(test_data), training_data, test_data, 10)
